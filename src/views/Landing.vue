<template>
  <div class="surface-0 flex justify-content-center">
    <div id="home" class="landing-wrapper">
      <div id="hero" class="flex flex-column pt-4 px-4 lg:px-8 overflow-hidden" style="
          background-image: linear-gradient(45deg, rgba(40, 46, 56, 0.7), rgba(89, 92, 102, 0.7)), url('/demo/images/landing/house.jpg');
        ...
        <div
          ...
          id="nav">
          <a class="flex align-items-center" href="/"> <img src="/layout/images/logo.png" height="50" class="mr-2" />
            <span class="text-200 font-medium text-2xl line-height-3 mr-8">RealEstate</span> </a>
          <a class="cursor-pointer block lg:hidden text-700">
            <i class="pi pi-bars text-4xl text-white" @click="visible = true"></i>
          </a>
          <div
            class="align-items-center flex-grow-1 justify-content-center hidden lg:flex absolute lg:static w-full left-0 px-6 lg:px-0 z-2"
            style="top: 100px">
            <ul
              class="list-none p-0 m-0 flex lg:align-items-center mx-auto select-none flex-column lg:flex-row cursor-pointer">
              <li>
                <router-link to="/" v-slot="{ navigate }">
                  <a @click="navigate()" class="flex m-0 md:ml-5 px-0 py-3 text-100 font-medium text-lg line-height-3">
                    <span>Home</span>
                  </a>
                </router-link>
              </li>
            ...
            <Sidebar v-model:visible="visible" style="opacity: 0.87; background-color: black" :showCloseIcon="false">
              <header>
                <a class="flex align-items-center" href="/"> <img src="/layout/images/logo.png" height="50"
                    class="mr-2" /> <span class="text-200 font-medium text-2xl line-height-3 mr-8">RealEstate</span> </a>
              </header>
              <ul class="list-none p-0 m-0 flex lg:align-items-center mx-auto select-none flex-column cursor-pointer">
                <li>
                  <router-link to="/" v-slot="{ navigate }">
                    <a @click="navigate()" class="flex m-0 md:ml-5 px-0 py-3 text-200 font-medium text-lg line-height-3">
                      <span>Home</span>
                    </a>
                  </router-link>
                </li>
              ...
              <div
                class="flex justify-content-between border-top-1 lg:border-top-none surface-border py-3 lg:py-0 mt-3 lg:mt-0">
                <router-link to="/pages/auth/login" v-slot="{ navigate }">
                  <Button role="link" label="Login"
                    class="p-button-text p-button-rounded border-none font-light line-height-2 text-200"
                    @click="navigate()" outlined></Button>
                </router-link>
                <router-link to="/pages/auth/register" v-slot="{ navigate }">
                  <Button role="link" label="Register"
                    class="p-button-text bg-orange-500 p-button-rounded border-none font-light line-height-2 text-white"
            ...
            <div v-if="isLogged === false">
              <div
                ...
                <router-link to="/pages/auth/register" v-slot="{ navigate }">
                  <Button role="link" label="Register"
                    class="p-button-text bg-primary p-button-rounded border-none font-light line-height-2 text-white"
                    @click="navigate()"></Button>
                </router-link>
              </div>
            </div>
            ...
            <div v-if="isLogged === true">
              <div
                ...
                <a class="flex align-items-center cursor-pointer text-600" @click="toggle">
                  ...
                  <span class="font-medium text-white text-lg mx-2">{{ this.userInfo.first_name }} {{
        ...
        <div class="py-4 px-4 mx-0 md:mx-6 lg:mx-8 lg:px-8 my-auto scalein animation-duration-1000 md:w-6 w-11">
          <p class="font-bold text-200 xl:text-7xl text-5xl">The Simplest Way to Find Property</p>
          <p class="font-medium xl:text-xl text-lg text-400">
            With over 20 years of experience, we are pioneers in the real estate business. We believe there's a perfect
            home for everybody, no matter the budget. That's why we always find the best homes for You.
          </p>
        </div>
      </div>
      ...
      <div id="features" class="py-4 px-4 lg:px-8 mt-5 mx-0 lg:mx-8 ">
        <div class="flex flex-column text-center mt-8 mb-6">
          <span class="title">Latest Property Listings</span>
        ...
        <div class="flex justify-content-center">
          <div class="grid px-2 md:px-0">
            <div class="col-12 lg:col-6 xl:col-4 flex justify-content-evenly" v-for="property in properties" :key="property">
              <Card class="transition-duration-500 shadow-6 hover:shadow-8" v-motion-slide-visible-top :delay="200" @click="() => $router.push('/pages/shop')"
                :hovered="{ y: -10 }">
                <template #header>
                  <img :src="'http://localhost/real-estate-api/storage/app/public/' + property.image_path" style="height: 18rem" class="border-round-top-2xl" />
                </template>
                <template #title>
      ...
      <div id="highlights" class="py-6 px-4 lg:px-8 mx-0 my-6 lg:mx-8" style="border-radius: 20px">
        <div class="flex flex-column text-center">
          ...
          <span class="text-600 text-2xl">Luxury Real Estate company</span>
        </div>
        ...
        <div class="grid mt-6 pb-2 md:pb-8" v-motion-slide-visible-right :delay="400">
          <div class="flex justify-content-center col-12 lg:col-6 p-0 flex-order-1 lg:flex-order-0"
            ...
            <img src="/demo/images/landing/house1.jpg" class="w-full border-round-xl mt-3" alt="mockup mobile" />
          </div>
          ...
          <div class="col-12 lg:col-6 my-auto flex flex-column lg:align-items-end text-center lg:text-right">
            ...
            <span class="text-s00 text-2xl line-height-3 ml-0 md:ml-2" style="max-width: 650px">
              We are a real estate company known for its expertise, reliability, and exceptional service. With years of
              experience, our dedicated team offers comprehensive solutions for selling properties.
              We pride ourselves on our vast network, enabling us to present a diverse portfolio of residential and
              commercial spaces.
            </span>
          </div>
        </div>

        <div class="grid my-8 pt-2 md:pt-8" v-motion-slide-visible-left :delay="400">
          <div class="col-12 lg:col-6 my-auto flex flex-column text-center lg:text-left lg:align-items-start">
            ...
            <span class="text-700 text-2xl line-height-3 mr-0 md:mr-2" style="max-width: 650px">
              From stylish apartments to spacious office buildings, we cater to various budgets and preferences. Trust Us
              for your real estate needs, as we strive to provide a seamless and satisfying experience for every client."
            </span>
          </div>

          <div class="flex justify-content-end flex-order-1 sm:flex-order-2 col-12 lg:col-6 p-0">
            <img src="/demo/images/landing/house2.jpg" class="w-full border-round-xl mt-3" />
          </div>
        </div>
      </div>
    </div>
